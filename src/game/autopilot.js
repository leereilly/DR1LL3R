...
export function drive(s) {
  ...
  const speed = s.speed * (double ? 1.4 : s.dr.boost);
  ...
  s.input.aim = true;
  const x = e.baseX + (e.amp || 0) * Math.sin((s.time + until) * e.w + e.phase);
  // Double Rainbow mirrors steering, so the autopilot mirrors its aim back.
  s.input.aimX = double ? VW - x : x;
}
